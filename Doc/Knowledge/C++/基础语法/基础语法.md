...
- &a是数组的指针，其类型为`int (*)[10]`（就是前面提到的**数组指针**），&a加1时，系统会认为是数组首地址加上整个数组的偏移（10个int型变量），值为**数组a尾元素后一个元素**的地址。
...
- C++中用来做控制态输入输出的**iostream类库**替代了标准C中的**stdio函数库**。
- C++中的try/catch/throw异常处理机制取代了标准C中的setjmp()和longjmp()函数。
- **C++可以重载**，C语言不允许。
- C++语言中，允许变量定义语句在**程序使用它之前的任何地方**，而C语言中，必须要在**函数开头部分**。而且C++不允许重复定义[全局变量](https://blog.csdn.net/SlowIsFastLemon/article/details/104109590#:~:text=%E5%AE%9A%E4%B9%89%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F-,1%20C%2B%2B%E4%B8%AD%E4%B8%8D%E5%85%81%E8%AE%B8%E9%87%8D%E5%A4%8D%E5%AE%9A%E4%B9%89%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F,%E5%90%8C%E4%B8%80%E4%B8%AA%E5%9C%B0%E5%9D%80%E7%A9%BA%E9%97%B4%E4%B8%8A%E3%80%82)，C语言也是做不到这一点的
...
-  完全面向对象
-  由于JVM，可移植性强；C++也可以在其他系统运行，但是**需要不同的编码**。
   -  原因：例如对一个数字，在windows下是大端存储，在unix中则为小端存储。
      -  Java程序一般都是生成**字节码**，在JVM里面运行得到结果
-  Java语言中将容易造成内存泄露的指针去掉，没有指针的概念，引入了**真正的数组**。
-  Java用**接口(Interface)技术**取代C++程序中的**抽象类**。二者有同样的功能，但是前者省却了在实现和维护上的复杂性

**垃圾回收**

- C++用析构函数回收垃圾，写C和C++程序时一定要注意**内存的申请和释放**
- Java语言不使用指针，内存的分配和回收都是自动进行的，程序员无须考虑内存碎片的问题

**应用场景**

- Java在**桌面程序**上不如C++实用，C++可以直接编译成exe文件，指针是c++的优势，可以直接对内存的操作，但同时具有危险性 。
- Java在**Web 应用**上具有C++ 无可比拟的优势，具有丰富多样的框架
...
- 如果不对成员指定公私有，struct默认是公有的，class则默认是私有的

- 而struct模式是public继承，class默认是private继承。
...
- 成员权限与内容
  - C中struct是没有权限的设置，且struct中只能是一些变量的集合体，可以封装数据却不可以隐藏数据

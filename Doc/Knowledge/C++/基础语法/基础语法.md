...
+ 设置栈指针
+ 初始化**静态`static`变量**和**`global`全局变量**，即**`.data`段**的内容
...
* 在对**非基本数据类型的对象**使用的时候，对象创建的时候还需要执行构造函数，销毁的时候要执行析构函数。
* 而malloc/free是库函数，是已经编译的代码，所以不能把构造函数和析构函数的功能强加给malloc/free，所以new/delete是必不可少的。



## 12、被free回收的内存是立即返还给操作系统吗？

不是的，被free回收的内存会首先被`ptmalloc`使用双链表保存起来，当用户下一次申请内存的时候，会尝试从这些内存中寻找合适的返回。这样就避免了频繁的系统调用，占用过多的系统资源。
...
## 14、define宏定义和函数和typedef区别？

|          |            define            |        typedef        |
| -------- | :--------------------------: | :-------------------: |
| 作用     | 定义**常量及书写复杂**的内容 |     定义类型别名      |
| 处理阶段 |           编译之前           | typedef是编译的一部分 |
| 类型检查 |              ×               |           √           |
| 语句？   |        非语句不加分号        |    语句，以`;`结束    |
|          |                              |                       |

- 注意对指针的操作，`#define p_char char *` 和 `typedef char * p_char`区别巨大。
...
- define只是**将宏名称进行替换**，在内存中会产生**多份相同的备份**。const在程序运行中只有**一份备份**，且可以执行==**常量折叠**==，能将复杂的的表达式计算出结果放入常量表

- 宏定义的数据没有分配内存空间，只是插入替换掉；const定义的变量**只是值不能改变**，但要分配内存空间。
...
- 不考虑类的情况
  - 隐藏。所有**加static的全局变量和函数**只能在**该文件所在的编译模块中**使用,如果不加则具有**全局可见性**，可以**在其他文件中使用**。
  - 默认初始化为0，包括**未初始化的<u>全局静态变量</u>与<u>局部静态变量</u>**，都存在**全局未初始化区**
  - 在函数内定义的**静态变量**，只进行一次初始化，始终存在；其作用域与局部变量相同，生命周期直到程序运行结束，即函数退出后仍然存在，但不能使用。
- 考虑类的情况
  - static成员变量：**只与类关联**，不与类的对象关联。定义时要分配空间，不能在类声明中初始化，==<u>只能</u>在**类定义体外部**初始化==，初始化时不需要标示为static；可以被**非static成员函数**任意访问。
  - static成员函数：不具有this指针，只能访问**类对象的**static成员变量和static成员函数；**不能被声明为const、虚函数和volatile**；可以被**非**static成员函数**任意**访问

**const**

- 不考虑类的情况

  - const常量**在定义时必须初始化**，之后无法更改

  - const**形参**可以**接收**const和非const类型的实参，例如：
...
- 考虑类的情况
  ...
  - const成员变量：==<u>不能</u>在**类定义外部初始化**==，**只能通过构造函数**初始化列表进行初始化，并且必须有构造函数——不同类对其const数据成员的值可以不同，所以**不能在类中声明时初始化**:confused:

  - const成员函数：const对象<u>只可以调用</u>const成员函数；非const对象都可以调用；

    **<u>只可以</u>**改变mutable数据的值（用该关键字声明的变量可以在const成员函数中被修改）

## ==15、变量声明和定义区别？==

|          |                声明                |   定义   |
| -------- | :--------------------------------: | :------: |
| what     | 把变量**声明的位置及类型**->编译器 |    -     |
| 内存空间 |               不分配               | 分配空间 |
| 相同变量 |                多处                |   一处   |
...
## 16.2、 String s;	//How to distinguish s.size() from s.length()?

不用区分这两[源码](https://www.cnblogs.com/lakeone/p/4764577.html)一样， length是因为沿用C语言的习惯而保留下来的，string类最初只有length，引入STL之后，为了兼容又加入了size

|                |                  `const char* s1 = "name";`                  |              `const string s2 = "hello";`               |
| -------------- | :----------------------------------------------------------: | :-----------------------------------------------------: |
| `sizeof(s_x)`  |                              8                               |                           32                            |
| `strlen(s_x)`  |                              4                               | `error: cannot convert 'const string' to 'const char*'` |
| `s_x.size()`   | `error: request for member 'size' in 's1', which is of non-class type 'const char*'` |                            5                            |
| `s_x.length()` | `error: request for member 'length' in 's1', which is of non-class type 'const char*'` |                            5                            |
|                |                                                              |                                                         |
...
int main(int argc, char const* argv[]) {
    ...
    const char* s1 = "name";           //如果指针变量不用const修饰：warning: ISO C++ forbids converting a string constant to 'char*'
    const string s2 = "hello";

    cout << sizeof(s1) << endl;        // 取的是指针s1的长度，是8
    cout << strlen(s1) << endl;        // 取的是这个字符串的长度，不包含结尾的 \0。大小是4
    // cout << s1.size() << endl;      // error: request for member 'size' in 's1', which is of non-class type 'const char*'
    // cout << s1.length() << endl;    // error: request for member 'length' in 's1', which is of non-class type 'const char*'

    cout << sizeof(s2) << endl;
    cout << sizeof(string) << endl;
    // cout << strlen(s2) << endl;       // error: cannot convert 'const string' {aka 'const std::__cxx11::basic_string<char>'} to 'const char*'
    cout << s2.size() << endl;
    cout << s2.length() << endl;
    return 0;
}
...
- 在const面前，要关注最后两个字：
- 指针**常量**：const修饰的是常量，所以const修饰的是*p; (`const int *p; / int const *p;`)
- 常量**指针**：const修饰的是指针，即const修饰p; (`int *const p;`)
...
- &a是数组的指针，其类型为`int (*)[10]`（就是前面提到的**数组指针**），&a+1时，系统会认为是系统会认为加了一个整个a数组，偏移了整个数组a的大小（也就是10个int的大小），值为**数组a尾元素<u>后一个元素</u>**的地址。
- ~~若`(int *)p` ，此时输出 `*p`时，其值为`a[0]`的值，因为被转为`int *`类型，解引用时按照`int`类型大小来读取。~~ | 没明白
...
int main(void)
{
    ...
    cout << "ptr1 about:" << endl;
    
    int* ptr1 = (int*)(&a + 1);// (&a + 1)，可以理解为将a数组整体包裹起来，并以之为单位加一；
    cout << a           << '\t' << a + 1        << endl;
    cout << &a          << '\t' << *(a + 1)     << endl;
    cout << ptr1        << '\t' << ptr1 + 1     << endl;
    cout << *ptr1       << '\t' << *(ptr1 + 1)  << endl;
    cout << *(ptr1 - 1) << '\t' << *(ptr1 - 2)  << endl;



    cout << "ptr2 about:" << endl;
    
    int(*ptr2)[5] = &a;
    cout << a       << '\t' << a + 1    << endl;
    cout << &a      << '\t' << ptr2     << endl;
    cout << &a + 1  << '\t' << ptr2 + 1 << endl;
...
- Python的基本数据类型只有数字，布尔值，字符串，列表，元组等等
...
|                                                              | C++                      | C                  |
| ------------------------------------------------------------ | ------------------------ | ------------------ |
| 内存分配                                                     | 运算符new/delete         | 库函数malloc/free  |
| 字符串                                                       | 字符串类                 | 字符数组处理函数   |
| IO                                                           | iostream类库             | stdio函数库        |
| 异常                                                         | try/catch/throw          | setjmp()/longjmp() |
| 重载                                                         | 允许                     | 不允许             |
| 变量定义语句                                                 | 程序使用他之前的任何地方 | 函数开头           |
| [重复定义全局变量](https://blog.csdn.net/SlowIsFastLemon/article/details/104109590#:~:text=%E5%AE%9A%E4%B9%89%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F-,1%20C%2B%2B%E4%B8%AD%E4%B8%8D%E5%85%81%E8%AE%B8%E9%87%8D%E5%A4%8D%E5%AE%9A%E4%B9%89%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F,%E5%90%8C%E4%B8%80%E4%B8%AA%E5%9C%B0%E5%9D%80%E7%A9%BA%E9%97%B4%E4%B8%8A%E3%80%82) | 允许                     | 不允许             |
| 传递                                                         | 值/指针/引用             | 值/指针            |
|                                                              |                          |                    |
...
| -------- | ------------------------- | -------------------- |
| 面向对象 | √                         | √                    |
| 可移植性 | 只需要生成字节码在JVM运行 | 需要针对平台进行编码 |
| 指针     | ×                         | √                    |
| 数组     | √                         | ×                    |
|          | 接口技术(Interface)       | 抽象类               |
| 垃圾回收 | 自动进行                  | 析构函数回收         |
...
- 对于**底层程序的编程以及控制**方面的编程，C++很灵活，因为有**句柄**的存在



## 22、C++中struct（公）和class（私）的区别
...
- 成员权限与内容
  - C中struct没有权限设置，可以封装数据却不可以隐藏数据
  - C++中，struct增加了访问权限，且可以和类一样有成员函数，成员默认访问权限（为了与C兼容）为public
- 作用
  - **struct作为类的一种特例是用来自定义数据结构的。**
  
    :confused: :arrow_down:
  
  - 一个结构标记声明后，在C中必须在结构标记前加上struct，才能做结构类型名（除：`typedef struct class{};`）
  
  - C++中**结构体名**可以直接作为**结构体类型名**使用，此外结构体struct在C++中被当作**类的一种特例**
...
浅拷贝只是**拷贝一个指针**，并**没有新开辟一个地址**，拷贝的指针和原来的指针指向同一块地址，如果原来的指针所指向的资源释放了，那么再释放浅拷贝的指针的资源就会出现错误。|皮之不存毛将焉附？

**深拷贝**

深拷贝不仅**拷贝值**，还**开辟出一块新的空间用来存放新的值**，即使原先的对象被析构掉，释放内存了也不会影响到深拷贝得到的值。*在自己实现拷贝赋值的时候，如果有指针变量的话是需要自己实现深拷贝的。*
...
- 在使用时，宏只做**简单字符串替换（编译前）**。
- 内联函数在编译时**直接将函数代码嵌入到目标代码**中，**省去函数调用的开销**来提高执行效率，并且**进行参数类型检查（编译时）**，具有**返回值**，可以实现**重载**。
...
- 作为**类成员接口函数**来读写类的**私有成员**或者**保护成员**，会提高效率。
...
- protected只能在类内部和其派生类中访问。

- private修饰的元素**只能在类内访问**。

（一）访问权限

**派生类**可以**继承**基类中**除了构造/析构、赋值运算符重载函数之外**的成员，但是这些成员的**访问属性**在派生过程中也是**可以调整**的，三种派生方式的访问权限如下表所示：注意外部访问并**不是真正的外部访问**，而是在通过**派生类的对象**对**基类成员**的访问。

![](https://cdn.jsdelivr.net/gh/forthespada/mediaImage1@1.6.4.2/202102/1564132255040.png)

派生类对基类成员的访问形象有如下两种：

- **内部访问**：由**派生类中**新增的成员函数对**从基类继承来的成员**的访问|内部成员函数-》基类继承成员
- **外部访问**：在**派生类外部**，通过**派生类的对象**对**从基类继承来的成员**的访问|外部对象-》基类继承成员

（二）继承权限

**public继承**

public继承的特点是**基类的<u>公有成员</u>和<u>保护成员</u>**作为派生类的成员时，**都保持<u>原有的状态</u>**，而基类的私有成员仍然是私有的，不能被这个派生类的子类所访问

**protected继承**

protected继承的特点是**基类的所有<u>公有成员</u>和<u>保护成员</u>**都成为**派生类的<u>保护成员</u>**，并且**只能**被它的派生类成员函数或**友元函数**访问，基类的私有成员仍然是私有的，访问规则如下表

![](https://cdn.jsdelivr.net/gh/forthespada/mediaImage1@1.6.4.2/202102/1564132983494.png)

**private继承**

private继承的特点是**基类的所有<u>公有成员</u>和<u>保护成员</u>**都成为**派生类的<u>私有成员</u>**，并不被它的派生类的子类所访问，基类的成员只能由自己派生类访问，无法再往下继承，访问规则如下表
...
**方式二：巧用union联合体** 利用内存对齐

```cpp
#include <iostream>
using namespace std;
// union联合体的重叠式存储，endian联合体占用内存的空间 为所有成员中 字节长度 的最大值
union endian
{
    int a;
    char ch;
...
{
    endian value;
    value.a = 0x1234; // 这才占了两个字节
    // a和ch共用4字节的内存空间
    if (value.ch == 0x12) //？这是什么操作？
        cout << "big endian" << endl;
...
volatile 关键字是一种类型修饰符，**被它声明的<u>类型变量</u>可以被某些编译器自主更改**，比如：操作系统、硬件或者其它线程等。遇到这个关键字声明的变量，编译器对访问该变量的代码就不再进行优化，从而可以提供对特殊地址的稳定访问。

 当要求使用 volatile 声明的变量的值的时候，**系统总是重新从它所在的内存读取数据**，即使它前面的指令刚刚从该处读取过数据。

volatile定义变量的值是易变的，每次用到这个变量的值的时候都要去**重新读取这个变量的值**，而不是读寄存器内的备份。**多线程中被几个任务共享的变量**需要定义为volatile类型。



**volatile 指针**

volatile 指针和 const 修饰词类似，const 有常量指针( `int *const p;`)

和指针常量(`const int *p;/int const *p;`)的说法，volatile 也有相应的概念
